<template>
  <div class="gallery" id="gallery">
		...
		<div class="galleryNameHeadingContainer">
			<h1 class="galleryNameHeading">{{ data.title }}</h1>
		</div>
		<div class="galleryNameGalleryContainer">
			<p v-for="(text, id) in data.texts" :key="'gallery-text-' + id">
        {{ text }}
			</p>
			<div class="galleryNameImage-Container row no-gutters">
				<div v-for="(data, id) in data.images" :id="'galleryNameContainer-' + id" :key="'galleryNameContainer-' + id" class="col-12 col-lg-6 col-xl-4 galleryNameImage-ContainerInner" @click="openModal(id)">
			...
			<div class="galleryNameImage-Container row no-gutters">
				<div v-for="(data, id) in data.images" :id="'Modal-' + id" :key="'Modal-' + id" class="col-12 col-lg-6 col-xl-4 galleryNameImage-ContainerInner galleryNameImageModal">
					<img class="galleryNameImage" :src="require(`@/assets/Images/${data}`)">
	...
	export default {
		...
		methods: {
			...
			next: function(){
				document.getElementById('Modal-' + this.lastId).classList.toggle('galleryNameImage-ContainerInner-Modal')
				this.lastId++
				if(this.lastId > this.data.images.length - 1){
					this.lastId = 0
				}
				document.getElementById('Modal-' + this.lastId).classList.toggle('galleryNameImage-ContainerInner-Modal')
			},
			previous: function(){
				...
				this.lastId--
				if(this.lastId < 0){
					this.lastId = this.data.images.length - 1
				}
				document.getElementById('Modal-' + this.lastId).classList.toggle('galleryNameImage-ContainerInner-Modal')
			}
		},
    created(){
			let imported = require('@/assets/Content/Termine/TermineSubpages/subpages.json')
